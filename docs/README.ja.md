...
**キャプチャ翻訳**では選択した翻訳 API のみを使用し、リクエストが失敗した場合はエラーを表示します。**リアルタイム翻訳**と**クイック翻訳**では、Google、Bing、Microsoft のいずれかが失敗したり応答が遅かったりすると、別の利用可能な API を試します。DeepL と OpenAI では自動バックアップを使用しません。
